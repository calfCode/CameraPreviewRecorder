...
void MVRecordingPreviewController::switchPreviewFilter(int filterType, byte* acvBuffer, int bufferSize) {
...
void MVRecordingPreviewController::configCamera() {
	...
	jclass jcls = env->GetObjectClass(obj);
	if (NULL != jcls) {
		jmethodID configCameraCallback = env->GetMethodID(jcls, "configCameraFromNative", "(I)Lcom/example/camerapreviewrecorder/camera/preview/CameraConfigInfo;");
